...
void InclusiveCross::planewave_int(numint::vector_d & result, double prnorm, double costheta,
  ...
  if((abs(structfactor)<1E-09)||isnan(structfactor)||std::isinf(structfactor)) {result[0]=0.; return; }
...
void InclusiveCross::calc_Azzinc(double &Azz, double &cross, double Q2,double x, double thetapol){
...
//   return 2.*PI*2.*massi/MASSD*result;
  ...
  F.f=InclusiveCross::Azzplanewave_int;
  int res=90;
  unsigned count=0;
//     res = numint::cube_romb(mdf,lower,upper,1.E-08,PREC,ret,count,0);
  res = numint::cube_adaptive(mdf,lower,upper,1.E-08,PREC,1E02,2E04,ret,count,0);
//     cout << res << " " << count << endl;
  Azz= 2.*PI*2.*massi/MASSD*ret[0];
  cross= 2.*PI*2.*massi/MASSD*ret[1];
}

void InclusiveCross::Azzplanewave_int(numint::vector_d & result, double prnorm, double costheta,
				   InclusiveCross& cross, double Q2, double x, double thetapol){
  
  result=numint::vector_d(2,0.);
  if(prnorm<1.E-03) { result[0]=result[1]=0.; return;}
  double Er=sqrt(cross.massr*cross.massr+prnorm*prnorm);
  double qvec=sqrt(Q2+pow(Q2/(2.*cross.massi*x),2.));
  //Wx^2> massi^2
  double lowerlimit= -(-cross.massi*cross.massi-Q2+pow(MASSD-Er,2.)-prnorm*prnorm+2.*(MASSD-Er)*Q2/(2.*cross.massi*x))/(2.*qvec*prnorm);
  ...
  if((abs(ResTandL)<1E-09)||isnan(ResTandL)||std::isinf(ResTandL)) {result[0]=result[1]=0.; return; }
  //no phi integration, symmetry is already used in getInclStructure()
...
//   /*if(cross.massr==MASSN) */cout << prnorm << " " << costheta << " " << result[0] << " " << result[0]/prnorm/prnorm << endl;
  ...
  for(int spini=-1;spini<=0;spini+=2){
    for(int spinr=-1;spinr<=1;spinr+=2){
      ...
      complex<double> wavezero=cross.wf->DeuteronPState(0, spini, spinr, TVector3(kin.GetPklab()*sintheta,
								     0.,
								     kin.GetPklab()*kin.GetCosthklab()));
      complex<double> waveplus=cross.wf->DeuteronPState(2, spini, spinr, TVector3(kin.GetPklab()*sintheta,
								     0.,
								     kin.GetPklab()*kin.GetCosthklab()));
								    
      dens1+=norm(wavemin)+norm(waveplus)-2.*norm(wavezero);
      dens2+=wavemin*conj(wavezero)+wavezero*conj(wavemin)-wavezero*conj(waveplus)-waveplus*conj(wavezero);
      dens3+=wavemin*conj(waveplus)+waveplus*conj(wavemin);
//       dens4+=wavemin*conj(wavezero)-wavezero*conj(wavemin)-wavezero*conj(waveplus)+waveplus*conj(wavezero);
...
void InclusiveCross::FSI_int(numint::vector_d & result, double prnorm, double costheta, double qt, 
		      double qphi, InclusiveCross& cross, double Q2, double x, double thetapol, 
		      size_t itt, size_t it2=0){
  ...
  double structfactor=cross.structure.getInclStructure(kin,MASSD-Er);
  if((abs(structfactor)<1E-09)||isnan(structfactor)||std::isinf(structfactor)) {result[0]=result[1]=0.; return; }
  //no phi integration, symmetry is already used in getInclStructure()
  ...
  if((abs(structfactor2)<1E-09)||isnan(structfactor2)||std::isinf(structfactor2)) {result[0]=result[1]=0.; return; }
...
void InclusiveCross::calc_AzzincFSI(double &Azz1, double &Azz2, double &fsi1, double &fsi2, double Q2,double x, double thetapol){
...
//   fsi2=results[1]*2.*PI*2.*massi/MASSD;
 //cout << fsi1 << " " << fsi2 << endl;
  ...
  //     res = numint::cube_romb(mdf,lower,upper,1.E-08,PREC,ret,count,0);
    res = numint::cube_adaptive(mdf,lower,upper,1.E-08,PREC,1E03,6E04,ret,count,0);
  //   cout << res << " " << count << endl;
    fsi1+= 2.*PI*2.*massi/MASSD*ret[2];
    fsi2+= 2.*PI*2.*massi/MASSD*ret[3];
    Azz1+= 2.*PI*2.*massi/MASSD*ret[0];
    Azz2+= 2.*PI*2.*massi/MASSD*ret[1];
  }
  return;
}

void InclusiveCross::AzzFSI_int(numint::vector_d & result, double prnorm, double costheta, double qt, 
		      double qphi, InclusiveCross& cross, double Q2, double x, double thetapol, size_t itt, size_t it2=0){
			
  result=numint::vector_d(4,0.);
  if(prnorm<1.E-03||qt<1.E-03) { result[0]=result[1]=result[2]=result[3]=0.; return;}
  double Er=sqrt(cross.massr*cross.massr+prnorm*prnorm);
  double qvec=sqrt(Q2+pow(Q2/(2.*cross.massi*x),2.));
  //Wx^2> massi^2
  double lowerlimit= -(-cross.massi*cross.massi-Q2+pow(MASSD-Er,2.)-prnorm*prnorm+2.*(MASSD-Er)*Q2/(2.*cross.massi*x))/(2.*qvec*prnorm);
  if(lowerlimit>costheta) {result[0]=result[1]=result[2]=result[3]=0.;return;}
  if(1.<lowerlimit){ result[0]=result[1]=result[2]=result[3]=0.; return;}
  double nu=Q2/(2.*cross.massi*x);
  cross.Wxprime2=MASSD*MASSD-Q2+cross.massr*cross.massr+2.*MASSD*(nu-Er)-2.*nu*Er+2.*qvec*prnorm*costheta;
  TKinematics2to2 kin("","",MASSD,cross.massr,sqrt(cross.Wxprime2),"qsquared:wlab:pklab",Q2,nu,prnorm);

  double structfactor=cross.structure.getavgStructure(kin,cross.electron,MASSD-Er);
  if((abs(structfactor)<1E-09)||isnan(structfactor)||std::isinf(structfactor)) {result[0]=result[1]=result[2]=result[3]=0.; return; }
  double ResTandL, ResTT, ResTL;
  cross.structure.getResponses(kin,cross.electron,ResTandL,ResTT,ResTL,MASSD-Er);
  if((abs(ResTandL)<1E-09)||isnan(ResTandL)||std::isinf(ResTandL)) {result[0]=result[1]=result[2]=result[3]=0.; return; }
  //no phi integration, symmetry is already used in getInclStructure()
  double chi=sqrt(kin.GetS()*kin.GetS()-2.*kin.GetS()*(cross.Wxprime2+cross.massr*cross.massr)
	      +pow(cross.massr*cross.massr-cross.Wxprime2,2.));
  complex<double> wave[6];
...
//     cout << (i/2)*2-2 << " " << (i%2)*2-1 << endl;
  ...
  vector<double> res_result;
  res_result=vector<double>(4,0.);
  cross.otherWx2=cross.resonances[itt]*cross.resonances[itt];
  double przprime = cross.get_prz_res(pt2,cross.otherWx2,kin);
  if(abs(przprime+1.E09)<1.E-02){result[0]=result[1]=result[2]=result[3]=0.; return;}
  double pprime = sqrt(pt2+przprime*przprime);
  double costhetaprime=przprime/pprime;
  double sinthetaprime=sqrt(pt2)/pprime;
  if(pprime<1.E-03) {costhetaprime=1.;sinthetaprime=0.;}
  ...
  if((abs(structfactor2)<1E-09)||isnan(structfactor2)||std::isinf(structfactor2)) {result[0]=result[1]=result[2]=result[3]=0.; return; }
  double ResTandL2, ResTT2, ResTL2;
  cross.structure.getResponses(kin2,cross.electron,ResTandL2,ResTT2,ResTL2,MASSD-Erprime);
  if((abs(ResTandL2)<1E-09)||isnan(ResTandL2)||std::isinf(ResTandL2)) {result[0]=result[1]=result[2]=result[3]=0.; return; }
  res_result[0]=res_result[1]=res_result[2]=res_result[3]=prnorm*prnorm
      /(abs(kin.GetKlab()-przprime/Erprime*(MASSD+nu))*32.*PI*PI*3.)
		*MASSD/sqrt(4.*(MASSD-Er)*Er*(MASSD-Erprime)*Erprime)*qt;
  res_result[2]*=sqrt(structfactor*structfactor2);
  res_result[3]*=sqrt(structfactor*structfactor2);
  //double t=2.*massr-2.*Er*Erprime+2.*prz*prz+2.*prt*pt*cos(phiprime-phi);
  //t = (ps-ps')^2
  double t=(Er-Erprime)*(Er-Erprime)-(prz-przprime)*(prz-przprime)-qt*qt;
  complex<double> wave2[6];
...
void InclusiveCross::FSI_int_off(numint::vector_d & result, double prt, double W, double qt, 
		      double qphi, InclusiveCross& cross, double Q2, double x, double thetapol, size_t itt, size_t itt2){
...
void InclusiveCross::calc_AzzincFSI_off(double &azz1, double &azz2, double &fsi1, double &fsi2, double Q2,double x, double thetapol){
...
//   cout << Wmax <<endl;
  ...
  mdf.param = &F;
  numint::vector_d ret(4,0.);
  F.f=InclusiveCross::AzzFSI_int_off;
  for(size_t it=0; it<resonances.size(); it++){
    F.it=it;
    /*for(size_t it2=0; it2<resonances.size(); it2++)*/{
      F.it2=it;
...
void InclusiveCross::AzzFSI_int_off(numint::vector_d & result, double prt, double W, double qt, 
		      double qphi, InclusiveCross& cross, double Q2, double x, double thetapol, size_t itt, size_t itt2){
			
  result=numint::vector_d(4,0.);
  if((qt<1.E-03)||(prt<1.E-03)) { result[0]=result[1]=result[2]=result[3]=0.; return;}
  ...
  //1st vertex structure function part.  On-shell with W!!!
  double prz=cross.get_prz_res(prt*prt,W*W,Q2,nu,qvec); //on-shell value for structure functions!! 1st vertex
  if(abs(prz+1.E09)<1.E-02){result[0]=result[1]=result[2]=result[3]=0.; return;}
  double prnorm=sqrt(prt*prt+prz*prz);
  double Er=sqrt(cross.massr*cross.massr+prnorm*prnorm);
  double xprime = Q2/(2.*((MASSD-Er)*nu+prz*qvec) -cross.massi*cross.massi+pow(MASSD-Er,2.)-prnorm*prnorm);
  if(xprime>1){result[1]=result[0]=0.;return;}
  ...
  cross.Wxprime2=cross.resonances[itt]*cross.resonances[itt];
  double prztilde=cross.get_prz_res(prt*prt,cross.Wxprime2,Q2,nu,qvec); //"on-shell" prop value (needed in wf evaluation)
  if(abs(prztilde+1.E09)<1.E-02){result[0]=result[1]=result[2]=result[3]=0.; return;}
  double prnormtilde=sqrt(prt*prt+prztilde*prztilde);
  double Ertilde=sqrt(cross.massr*cross.massr+prnormtilde*prnormtilde);
  if((Ertilde>MASSD)){result[1]=result[0]=0.;return;}
  double costhetatilde=prztilde/prnormtilde;
  ...
  cross.otherWx2=cross.resonances[itt2]*cross.resonances[itt2];
  double prztildeprime=cross.get_prz_res(pt*pt,cross.otherWx2,Q2,nu,qvec);
  if(abs(prztildeprime+1.E09)<1.E-02){result[0]=result[1]=result[2]=result[3]=0.; return;}
  double ptildeprime = sqrt(pt*pt+prztildeprime*prztildeprime);
  double costhetatildeprime=prztildeprime/ptildeprime;
  double sinthetatildeprime=pt/ptildeprime;
  if(ptildeprime<1.E-03) {costhetatildeprime=1.;sinthetatildeprime=0.;}
...
//     azz+=(((i/4)*2-2)==0?-2.:1.)*wave[i]*conj(wave2[i]);
  ...
  double chi=sqrt(kin.GetS()*kin.GetS()-2.*kin.GetS()*(cross.Wxprime2+cross.massr*cross.massr)
		+pow(cross.massr*cross.massr-cross.Wxprime2,2.));
  cross.sigma=InclusiveCross::sigmaparam(cross.Wxprime2,kin.GetQsquared());
  result[0]*= offshellness*imag(cross.scatter(t)*2.*azz)*chi; //factor 2 due to symmetry
  result[2]*= offshellness*imag(cross.scatter(t)*2.*temp*sqrt(structfactor*structfactor2))*chi; //factor 2 due to symmetry
...
//     +pow(cross.massr*cross.massr-cross.Wxprime2,2.) << endl;
  ...
  double chi2=sqrt(kin.GetS()*kin.GetS()-2.*kin.GetS()*(cross.otherWx2+cross.massr*cross.massr)
    +pow(cross.massr*cross.massr-cross.otherWx2,2.));
  result[1]*=offshellness*imag(cross.scatter(t)*2.*conj(azz))*chi2; //factor 2 due to symmetry
  result[3]*=offshellness*imag(cross.scatter(t)*2.*conj(temp)*sqrt(structfactor*structfactor2))*chi2; //factor 2 due to symmetry
...
void InclusiveCross::FSI_int_Gauss_off(numint::vector_d & result, double mass, double prt, double W, double qt, 
  ...
  cross.setResonance(itt,mass);
  FSI_int_off(result,prt,W,qt,qphi,cross,Q2,x,0.,itt,itt2);
  double coef=exp(-(mass-centrals[itt])*(mass-centrals[itt])/(2.*widths[itt]*widths[itt]))/(widths[itt]*sqrt(2.*PI));
  result[0]*=coef;
  result[1]*=coef;
  return;
...
void InclusiveCross::calc_AzzincFSI_uniform(double &azz1, double &azz2, double &fsi1, double &fsi2, double Q2,double x, 
					    std::vector<double> & centrals, std::vector<double> & widths){
  ...
  mdf.param = &F;
  numint::vector_d ret(4,0.);
  F.f=InclusiveCross::AzzFSI_int_uniform;
  for(size_t it=0; it<resonances.size(); it++){
    numint::array<double,5> lower = {{centrals[it]-widths[it],0.,-1.,0.,0.}};
    numint::array<double,5> upper = {{centrals[it]+widths[it],1.E03,1.,1.E03,2.*PI}};
    F.it=it;
...
void InclusiveCross::AzzFSI_int_uniform(numint::vector_d & result, double mass, double prnorm, double costheta, double qt, 
		      double qphi, InclusiveCross& cross, double Q2, double x, size_t itt, size_t it2,
		      std::vector<double> & centrals, std::vector<double> & widths){

  cross.setResonance(itt,mass);
  AzzFSI_int(result,prnorm,costheta,qt,qphi,cross,Q2,x,itt,it2);
  double coef=1./(2.*widths[itt]);
  result[0]*=coef;
  result[1]*=coef;
  result[2]*=coef;
...
void InclusiveCross::calc_AzzincFSI_uniform_off(double &azz1, double &azz2, double &fsi1, double &fsi2, double Q2,double x, 
					      std::vector<double> & centrals, std::vector<double> & widths){
...
//   cout << Wmax <<endl;
  ...
  mdf.param = &F;
  numint::vector_d ret(4,0.);
  F.f=InclusiveCross::AzzFSI_int_uniform_off;
  for(size_t it=0; it<resonances.size(); it++){
    numint::array<double,5> lower = {{centrals[it]-widths[it],0.,massi,0.,0.}};
    numint::array<double,5> upper = {{centrals[it]+widths[it],1.E03,Wmax,1.E03,2.*PI}};
    F.it=it;
...
void InclusiveCross::AzzFSI_int_uniform_off(numint::vector_d & result, double mass, double prt, double W, double qt, 
		      double qphi, InclusiveCross& cross, double Q2, double x,size_t itt, size_t itt2,
		      std::vector<double> & centrals, std::vector<double> & widths){

  cross.setResonance(itt,mass);
  AzzFSI_int_off(result,prt,W,qt,qphi,cross,Q2,x,0.,itt,itt2);
  double coef=1./(2.*widths[itt]);
  result[0]*=coef;
  result[1]*=coef;
  result[2]*=coef;
