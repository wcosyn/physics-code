...
InclusiveCrossRes::InclusiveCrossRes(bool proton, string strucname, string wavename, 
				     TElectronKinematics &elec, int sym, int offshell, bool fixpropp)
:massi(proton? MASSP:MASSN),
massr(proton? MASSN:MASSP),
symm(sym),
fixprop(fixpropp),
...
//       double t = (Ex-Exprime)*(Ex-Exprime)-(prz-przprime)*(prz-przprime)-qt*qt;
  ...
  //   
    ...
    if(Erprime>MASSD) {results[1]=0.; return;}
//     double Ex = sqrt(getResonance_vec()[res2].getMass2()+(pkin2->GetKlab()-prz)*(pkin2->GetKlab()-prz)+prt*prt);
//     double Exprime = sqrt(getResonance_vec()[res1].getMass2()+(pkin2->GetKlab()-przprime)*(pkin2->GetKlab()-przprime)+pt2);
//     double t = (Ex-Exprime)*(Ex-Exprime)-(prz-przprime)*(prz-przprime)-qt*qt;

    double t = getResonance_vec()[res1].getMass2() + getResonance_vec()[res2].getMass2()
	-2.*(-pkin2->GetQsquared()+2.*MASSD*pkin2->GetWlab()+MASSD*MASSD
	  -(MASSD+pkin2->GetWlab())*(Er+Erprime)+pkin2->GetKlab()*(prz+przprime)
	+Er*Erprime-prt*sqrt(pt2)*cosphiprime-prz*przprime);
...
// 	double t = (Ex-Exprime)*(Ex-Exprime)-(prz-przprime)*(prz-przprime)-qt*qt;
  ...
  //   
    ...
    double Erprime=sqrt(massr*massr+pprime*pprime);
    if(Erprime>MASSD) {results[1]=0.; return;}
//     double Ex = sqrt(getResonance_vec()[res2].getMass2()+(pkin1->GetKlab()-prz)*(pkin1->GetKlab()-prz)+prt*prt);
//     double Exprime = sqrt(getResonance_vec()[res1].getMass2()+(pkin1->GetKlab()-przprime)*(pkin1->GetKlab()-przprime)+pt2);
//     //double t=2.*massr-2.*Er*Erprime+2.*prz*prz+2.*prt*pt*cos(phiprime-phi);
//     //double t=(Er-Erprime)*(Er-Erprime)-(prz-przprime)*(prz-przprime)-qt*qt;
//     double t = (Ex-Exprime)*(Ex-Exprime)-(prz-przprime)*(prz-przprime)-qt*qt;
