...
//   double qvec=sqrt(Q2+pow(Q2/(2.*cross.massi*x),2.));
//   double nu=Q2/(2.*cross.massi*x);
//   //X' prime at structure function evaluation vertex part
//   cross.Wxprime2=cross.resonances[itt2]*cross.resonances[itt2];
//   double prztilde=cross.get_prz_res(prt*prt,cross.Wxprime2,Q2,nu,qvec); //on-shell value (needed in wf evaluation)
//   if(abs(prztilde+1.E09)<1.E-02){result[0]=result[1]=0.; return;}
//   double prnorm=sqrt(prt*prt+prztilde*prztilde);
//   double Er=sqrt(cross.massr*cross.massr+prnorm*prnorm);
//   double Ex=MASSD+nu-Er;
//   double xprime = Q2/(2.*((MASSD-Er)*nu+prztilde*qvec) -cross.massi*cross.massi+pow(MASSD-Er,2.)-prnorm*prnorm);
//   if((Er>MASSD)||(xprime>1)){result[1]=result[0]=0.;return;}
//   double costheta=prztilde/prnorm;
//   double invX = Ex*Ex-qvec*qvec-prnorm*prnorm+2.*qvec*prztilde; //invariant mass X
...
// 					  TVector3(prnorm*sqrt(1.-costheta*costheta),0.,prnorm*costheta)); 
//   }
//   double sinqphi,cosqphi;
//   sincos(qphi,&sinqphi,&cosqphi);
//   double pt2=prt*prt+qt*qt-2.*prt*qt*cosqphi;  
...
//   }
//   cross.otherWx2=cross.resonances[itt]*cross.resonances[itt];
//   double prztildeprime=cross.get_prz_res(pt2,cross.otherWx2,kin);
//   if(abs(prztildeprime+1.E09)<1.E-02){result[0]=result[1]=0.; return;}
//   double pprime = sqrt(pt2+prztildeprime*prztildeprime);
//   double costhetaprime=prztildeprime/pprime;
//   double sinthetaprime=sqrt(pt2)/pprime;
//   if(pprime<1.E-03) {costhetaprime=1.;sinthetaprime=0.;}
//   double Erprime=sqrt(cross.massr*cross.massr+pprime*pprime);
//   TKinematics2to2 kin2("","",MASSD,cross.massr,sqrt(cross.otherWx2),"qsquared:wlab:pklab",Q2,nu,pprime);
...
//   if((abs(structfactor2)<1E-09)||isnan(structfactor2)||isinf(structfactor2)) {result[0]=result[1]=0.; return; }
//   result[0]=result[1]=prt*W*sqrt(structfactor*structfactor2)/(abs(qvec-prztilde/Er*(MASSD+nu))
// 			*abs(qvec-prztildeprime/Erprime*(MASSD+nu))*32.*PI*PI*3.)
// 		      *MASSD/sqrt(4.*(MASSD-Er)*Er*(MASSD-Erprime)*Erprime)*qt;
//   double Exprime=MASSD+nu-Erprime;
//   xprime=kin.GetQsquared()/(2.*((MASSD-Erprime)*kin.GetWlab()+prztildeprime*kin.GetKlab()) 
//       -cross.massi*cross.massi+pow(MASSD-Erprime,2.)-pprime*pprime);
//   if((Erprime>MASSD)||xprime>1) {result[0]=result[1]=0.; return;}
//   //double t=2.*massr-2.*Er*Erprime+2.*prz*prz+2.*prt*pt*cos(phiprime-phi);
//   //t = (ps-ps')^2
//   double t=(Er-Erprime)*(Er-Erprime)-(prztilde-prztildeprime)*(prztilde-prztildeprime)-qt*qt;
// //   cout << t << " " << t2 << " " << sqrt(cross.Wxprime2) << " " << sqrt(cross.otherWx2) << " " << Ex << " " << Exprime << endl;
//   
//   complex<double> wave2[6];
//   TVector3 vecprime(pprime*sinthetaprime*cosphiprime,pprime*sinthetaprime*sinphiprime,prztildeprime);
//   for(int i=0;i<6;i++){
