...
struct Ftor_2vector_general {
  ...
  static void exec(const numint::array<double,2> &x, void *param, numint::vector_z &ret) {
    ...
    p.f(ret,x[0],x[1], p.xi, p.mandelstam_t, p.scale, p.psq, p.Qsq, p.helampindex, p.omegapol,  p.gammapol, *p.pobj);
  }
  double xi; ///< [] skewness
  double mandelstam_t; ///< [GeV^2] momentum transfer, taken at t_min for now
  ...
  void (*f)(numint::vector_z &, double u, double z, double xi, double mandelstam_t, double scale, double psq, double Qsq, 
            int helampindex, TwoVector_Deut::Omega_pol rhopol, TwoVector_Deut::Photon_pol gammapol, TwoVector_Deut& twovector);
...
static void integrandum_omega_general(numint::vector_z &, double u, double z, double xi, double mandelstam_t, double scale, 
