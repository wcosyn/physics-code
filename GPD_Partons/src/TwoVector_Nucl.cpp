...
double TwoVector_Nucl::getCross_gammaL_rhoL(const double scale, const double xi, const double Q2, const double psq){
    ...
    //cout << 16.*PI*PI*alpha_s*frhoplus*xi*sqrt(1-xi*xi)*CF/Nc/psq/psq << endl;
    ...
    //cout << 1./256./pow(PI,3.)/xi/(1.-xi*xi) << endl;
    double result_L=pow(integral[0],2.)/256./pow(PI,3.)/xi/(1.-xi*xi); // Enberg et al Eq (12)
    //cout << Qsq << " " << xi << " " << psq << " " << result_L*0.389379E06 << endl;  // factor to go from GeV-6 to nb GeV-4
    //cout << pow(16.*PI*PI*alpha_s*frhoplus*xi*sqrt(1-xi*xi)*CF/Nc/psq/psq*2.*PI*alpha_s*sqrt(Q2)/Nc/sqrt(2.)*frho0*sqrt(ALPHA*4.*PI),2.)/256./pow(PI,3.)/xi/(1.-xi*xi)*0.389379E06 << " " << kk*kk << " " ;
...
double TwoVector_Nucl::getCross_gammaT_rhoL(const double scale, const double xi, const double Q2, const double psq){
    ...
    //cout << "pre1 " <<  16.*PI*PI*alpha_s*frhoplus*xi*sqrt(1-xi*xi)*CF/Nc/psq/psq << " " << alpha_s << " " << frhoplus << " " << xi << " " << psq << endl;
    ...
    double result_T=pow(integral[0],2.)/256./pow(PI,3.)/xi/(1.-xi*xi); // Enberg et al Eq (12)
    //cout << "pre3 " << 1./256./pow(PI,3.)/xi/(1.-xi*xi) << endl;
    cout << pow(16.*PI*PI*alpha_s*frhoplus*xi*sqrt(1-xi*xi)*CF/Nc/psq/psq*PI*alpha_s*sqrt(psq)/Nc/sqrt(2.)*frho0*sqrt(ALPHA*4.*PI),2.)/256./pow(PI,3.)/xi/(1.-xi*xi)*0.389379E06 << " " << kk*kk << " " ;
...
double TwoVector_Nucl::getCross_gammaL_rhoT(const double scale, const double xi, const double Q2, const double psq){
    ...
    for(int spinin=-1;spinin<=1;spinin+=2){        
        for(int spinout=-1;spinout<=1;spinout+=2){        
            ...
            //cout << "pre3 " << 1./256./pow(PI,3.)/xi/(1.-xi*xi) << endl;
            result_L+=pow(integral[0],2.)/256./pow(PI,3.)/xi/(1.-xi*xi); // Enberg et al Eq (12)
...
double TwoVector_Nucl::getCross_gammaT_rhoT(const double scale, const double xi, const double Q2, const double psq){
    ...
    for(int spinin=-1;spinin<=1;spinin+=2){        
        for(int spinout=-1;spinout<=1;spinout+=2){        
            ...
            integral[0] *= 16.*PI*PI*alpha_s*frhoplus*xi*sqrt(1-xi*xi)*CF/Nc/psq/psq; //prefactors Eq (14) Enberg et al.
            //cout << "pre1 " << 16.*PI*PI*alpha_s*frhoplus*xi*sqrt(1-xi*xi)*CF/Nc/psq/psq << " " << frhoplus << endl;
            integral[0] *= PI*alpha_s*sqrt(psq)/Nc/sqrt(2.)*frho0*sqrt(ALPHA*4.*PI); //prefactors Eq (17) Enberg et al.
            //cout << "pre2 " << PI*alpha_s*sqrt(psq)/Nc/sqrt(2.)*frho0*sqrt(ALPHA*4.*PI) << endl;
            //cout << "pre3 " << 1./256./pow(PI,3.)/xi/(1.-xi*xi) << endl;
            // cout << PI*alpha_s*sqrt(psq)/Nc/sqrt(2.)*frho0*sqrt(ALPHA*4.*PI) << endl;
            result_T+=pow(integral[0],2.)/256./pow(PI,3.)/xi/(1.-xi*xi); // Enberg et al Eq (12)
