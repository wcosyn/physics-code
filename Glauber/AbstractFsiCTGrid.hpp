...
public:
   ...
   */
  AbstractFsiCTGrid(const int r_grid, const int th_grid, const int phi_grid, MeanFieldNucleus *pnucl, string dir);
  ...
  /*!returns the value of the fsi+ct grid for a certain situation at coordinate (r,theta,phi) */
  complex<double> getFsiCtGridFull_interpvec(const TVector3 &rvec);
  /*!returns the value of the fsi+ct grid for a certain situation at coordinate (r,theta,phi) */
  complex<double> getFsiCtGridFull_interp3(const double r, const double theta, const double phi);
  /*!returns the value of the fsi+ct grid for a certain situation at coordinate (theta,phi), r has been set previously */
  complex<double> getFsiCtGridFull_interp2(const double theta, const double phi);
