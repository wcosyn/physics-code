...
public:
   ...
   */
  AbstractFsiGrid(const int r_grid, const int th_grid, const int phi_grid, MeanFieldNucleus *pnucl, string dir);
  ...
  int getThgrid() const;/*!<  returns the gridsize in theta*/
  ...
  void setThinterp(const double theta); /*!< sets the thindex variable used in the 3d interpolation */
  ...
  /*!returns the value of the fsi grid for a certain situation at coordinate (r,theta,phi) */
  complex<double> getFsiGridFull_interpvec(const TVector3 &rvec); 
  /*!returns the value of the fsi grid for a certain situation at coordinate (r,theta,phi) */
  complex<double> getFsiGridFull_interp3(const double r, const double theta, const double phi); 
  /*!returns the value of the fsi grid for a certain situation at coordinate (theta,phi), r has been set previously */
  complex<double> getFsiGridFull_interp2(const double theta, const double phi);
  ...
  /*! 3d interpolation of a grid after a certain coordinate (r,theta,phi) has been set! */
...
private:
  ...
  int thgrid; /*!<  the gridsize in theta*/
