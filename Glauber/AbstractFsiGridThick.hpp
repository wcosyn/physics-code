...
public:
  ...
  std::complex<double> getFsiSrcGridFull_interpvec(const TVector3 & rvec);
 /*!returns the value of the fsi+src grid for a certain situation at coordinate (r,costheta,phi) */
  std::complex<double> getFsiSrcGridFull_interp3(const double r, const double costheta, const double phi);
  /*!returns the value of the fsi+src grid for a certain situation at coordinate (theta,phi), r has been set previously */
  std::complex<double> getFsiSrcGridFull_interp2(const double costheta, const double phi);
  /*!returns the value of the fsi+src grid for a certain situation at coordinate (phi), r&theta have been set previously */
  std::complex<double> getFsiSrcGridFull_interp1(const double phi);
  ...
  virtual std::complex<double> getFsiSrcGridFull_interp()=0;
