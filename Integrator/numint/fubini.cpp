/**
 * @file fubini.cpp
 * @brief Fubini type multi-dimensional scalar integration
 *
...
struct quad {
  typedef int (* type)(const function<T> &f, double a, double b,
                       double epsabs, double epsrel, T &, unsigned &neval);
};

/**
   The set of integration parameters for multi-dimensional integration
*/
...
struct IntegrationParameters {
  ...
  IntegrationParameters (const mdfunction<T,N> &F, const numint::array<double,N> &A,
                         ...
                         unsigned NEVAL, int ERROR, typename quad<T>::type QUAD) 
    : func(F), a(A), b(B), epsabs(EPSABS), epsrel(EPSREL),
      neval(NEVAL), error(ERROR), quad(QUAD) {}
...
struct RecursiveFubini {
  static void exec(double xm, void *param, T &ret) {
    ...
    int error;
    T result;
    error = (*(p->quad))(f,p->a[M-1],p->b[M-1],p->epsabs,p->epsrel,result,dummy);
...
int _fubini_(const mdfunction<T,N> &f, const numint::array<double,N> &a,
             const numint::array<double,N> &b, double epsabs, double epsrel,
             T &result, unsigned &neval, typename quad<T>::type quad) {
  
  int error = SUCCESS;
  
  IntegrationParameters<T,N> param(f,a,b,epsabs,epsrel,neval,error,quad);
...
#define FUBINI_TYPE(CC)                                 \
  FUBINI_LOOP(CC,double); FUBINI_LOOP(CC,dcomplex);
