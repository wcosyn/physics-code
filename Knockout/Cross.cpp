...
Cross::Cross(TElectronKinematics &elec, MeanFieldNucleusThick *pnucleus, string dir)
...
/*  kinfactors[0]=pow(Q2overkk,2.);
  ...
  kinfactors[3]=Q2overkk*pow(cos(phi),2)+tan2;*/
  kinfactors[0]=pow(Q2overkk,2.);
  kinfactors[1]=tan2+Q2overkk/2.;
...
/*      complex<double> jmin=reacmodel->getMatrixEl(kin,spin,-1,shellindex,m,CT,pw);
	...
	cout << m << " " << spin << " " << j0 << endl;
      ...
      response[1]+=norm(jmin)+norm(jplus);
      response[2]+=2.*real(conj(jplus)*jmin);
      response[3]+=2.*real(j0*(jplus-jmin));
      response[4]+=norm(jplus)-norm(jmin);
      response[5]+=2.*imag(j0*conj(jplus-jmin));*/
	for(int i=0;i<2;i++){
