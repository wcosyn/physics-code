...
double Cross::getDiffCross(TKinematics2to2 &kin, int current, int thick, int SRC, int CT, int pw, int shellindex, double phi){
  ...
  for(int m=-pnucl->getJ_array()[shellindex];m<=pnucl->getJ_array()[shellindex];m+=2){
	...
	response[0][4]+=norm(J(i,2))-norm(J(i,1));
...
void  Cross::getAllDiffCross(std::vector<double> &cross, TKinematics2to2 &kin, int current, int shellindex, int thick, double phi, int maxEval){
  ...
  for(int m=-pnucl->getJ_array()[shellindex];m<=0/*pnucl->getJ_array()[shellindex]*/;m+=2){
	...
	for(int j=0;j<total;j++){
	  ...
	  response[j][3]+=2.*real(conj(J[j](i,0))*(J[j](i,2)-J[j](i,1)));
	  response[j][4]+=norm(J[j](i,2))-norm(J[j](i,1));
	  response[j][5]+=2.*imag(J[j](i,0)*conj(J[j](i,2)-J[j](i,1)));
	}
      }
  }
