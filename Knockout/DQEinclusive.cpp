...
void DQEinclusive::planewave_int(numint::vector_d & result, double pperp,
  ...
  if(cross.get_prz(prz,pperp,Q2,nu,qvec)){ //prz evaluation is performed!!
    for(size_t it=0;it<1/*prz.size()*/;it++){      
...
//       double costheta=prz[it]/prnorm;
	...
	for(int spinn=-1;spinn<=1;spinn+=2){
	  for(int spini_in=-1;spini_in<=1;spini_in+=2){
	    ...
	    for(int spini_out=-1;spini_out<=1;spini_out+=2){
	      ...
	      //cross term
	      complex<double> currentout0prime=conj((spinn==-1?us_down:us_up)*J0prime*(spini_out==-1?uiprime_down:uiprime_up));
	      complex<double> currentoutplusprime=conj((spinn==-1?us_down:us_up)*Jplusprime*(spini_out==-1?uiprime_down:uiprime_up));
	      complex<double> currentoutminprime=conj((spinn==-1?us_down:us_up)*Jminprime*(spini_out==-1?uiprime_down:uiprime_up));
	      for(int M=-2;M<=2;M+=2){
		for(int spinr=-1;spinr<=1;spinr+=2){
		  ...
		  //cross
		  if(Erprimenorm<MASSD-500.) res1+=real(nu*(Q2*Q2/pow(qvec,4.)*currentin0*currentout0prime
				      +Q2/(2.*qvec*qvec)*(currentinmin*currentoutminprime+currentinplus*currentoutplusprime))*
				  cross.getDeutwf()->DeuteronPState(M, spini_in, spinr, TVector3(pperp*cosphi,pperp*sinphi,prz[it]))
				  *conj(cross.getDeutwf()->DeuteronPState(M, spini_out, spinr, TVector3(-pperp*cosphi,-pperp*sinphi,qvec-prz[it]))));
	...
	if(Erprimenorm<MASSD-500.) res1*=-pperp*MASSD/(2.*sqrt((MASSD-Ernorm)*(MASSD-Erprimenorm)))/2./abs(qvec-prz[it]/Ernorm*(MASSD+nu))
...
void DQEinclusive::calc_F2DincFSI(double &fsi1, double &fsi2, double Q2,double x, int current){
...
void DQEinclusive::FSI_int(numint::vector_d & result, double pperp1, double qt, 
  ...
  if(cross.get_prz(prz1,pperp1,Q2,nu,qvec)){ //prz evaluation is performed!!
    for(size_t it1=0;it1<1/*prz.size()*/;it1++){      
...
//       double costheta=prz[it]/prnorm;
	...
	if(cross.get_prz(prz2,pperp2,Q2,nu,qvec)){ //prz evaluation is performed!!
	  for(size_t it2=0;it2<1/*prz.size()*/;it2++){      
	    ...
	    if(Ernorm<MASSD-500.){
	      ...
	      FourVector<double> pn2=pi1+q; //pn2=pi2+q
	      double t=pow(Ernorm-Erprimenorm,2.)-pow(prz1[it1]-prz2[it2],2.)-qt*qt;
	      double t_cross=pow(pn2[0]-Ernorm,2.)-pow(pn2[3]-prz1[it1],2.)-pperp1*pperp1;
	      double t_cross2=pow(pn1[0]-Erprimenorm,2.)-pow(pn1[3]-prz2[it2],2.)-pperp2*pperp2;
	      ...
	      //crossed term
	      GammaStructure J0prime_cross = cross.getFFactorsdiff()->getCC(current, q, pi2, pn2)*polVector0;
	      GammaStructure Jplusprime_cross = cross.getFFactorsdiff()->getCC(current, q, pi2, pn2)*polVectorPlus;
	      GammaStructure Jminprime_cross = cross.getFFactorsdiff()->getCC(current, q, pi2, pn2)*polVectorMin;	     
	      
	      Matrix<1,4> un2_down_cross=TSpinor::Bar(TSpinor(pn2,cross.getMassr(),TSpinor::Polarization(0.,0.,TSpinor::Polarization::kDown),TSpinor::kDoubleMass));
	      Matrix<1,4> un2_up_cross=TSpinor::Bar(TSpinor(pn2,cross.getMassr(),TSpinor::Polarization(0.,0.,TSpinor::Polarization::kUp),TSpinor::kDoubleMass));
	      TSpinor ui2_down_cross(pi2,cross.getMassr(),TSpinor::Polarization(0.,0.,TSpinor::Polarization::kDown),TSpinor::kDoubleMass);
	      TSpinor ui2_up_cross(pi2,cross.getMassr(),TSpinor::Polarization(0.,0.,TSpinor::Polarization::kUp),TSpinor::kDoubleMass);
		...
		for(int spini_in=-1;spini_in<=1;spini_in+=2){
		  ...
		  for(int spini_out=-1;spini_out<=1;spini_out+=2){
			...
			res0+=nu*(Q2*Q2/pow(qvec,4.)*currentin0*currentout0+Q2/(2.*qvec*qvec)
			  *(currentinmin*currentoutmin+currentinplus*currentoutplus))*wf*rescatter.scatter(t,0);
					
			//cross
			res1+=nu*(Q2*Q2/pow(qvec,4.)*currentin0*currentout0_cross+Q2/(2.*qvec*qvec)*
			  (currentinmin*currentoutmin_cross+currentinplus*currentoutplus_cross))*wf*rescatter.scatter(t_cross,0);
	      ...
	      /abs(qvec-prz2[it2]/Erprimenorm*(MASSD+nu))*chi*rescatter.scatter(t,0);
	      //minus sign because of wave function!!
	      res1*=pperp1*qt*MASSD/(2.*sqrt((Ernorm*Erprimenorm)*(MASSD-Ernorm)*(MASSD-Erprimenorm)))
