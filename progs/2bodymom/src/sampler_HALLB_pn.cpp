 ...
 */
void HALLB::generateKinematics_pn(std::vector<struct Event>& events, unsigned n, MeanFieldNucleusThick& nuc, int seed) {
	srand(seed);
	std::default_random_engine generator;
	generator.seed(seed);
	std::uniform_int_distribution<int> shell_distribution_p(0,nuc.getPLevels()-1); // shell index generator, bounds inclusive
	std::uniform_int_distribution<int> shell_distribution_n(nuc.getPLevels(),nuc.getTotalLevels()-1);
	events.clear();
	...
	while (logger.accept < n){
		...
		get_Pcm(Pcm);
		TVector3 k1  = TVector3(0.,0.,300. + 1200.*rand()/RAND_MAX );
		/** shellindices **/
		int shellindex1 = shell_distribution_p(generator); // get a random shellindex1
		int shellindex2 = shell_distribution_n(generator); // get a random shellindex2
		logger.tries++;
		Event e;
		e.xB          = xB;
		e.Q2          = Q2;
		...
		e.mass1       = MASSP;
		e.mass2       = MASSN;
		e.type1       = 0; // 0 is for proton
		e.type2       = 1; // 1 is for neutron
		...
		if (valid) { // valid, q has physical value
			...
			double M_Amin2 = nuc.getMassA_min_pn()+nuc.getExcitation()[shellindex1]+nuc.getExcitation()[shellindex2];
...
void HALLB::get_Pcm(TVector3& Pcm){
	Pcm.SetX(-1000.0 + 2000.*(double) rand()/RAND_MAX);
	Pcm.SetY(-1000.0 + 2000.*(double) rand()/RAND_MAX);
	Pcm.SetZ( -400.0 + 1400.*(double) rand()/RAND_MAX);
	/*
