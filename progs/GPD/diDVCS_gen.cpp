...
void GenerateEvents(double EneL, double EneH, int nevents, int ndec, DiDVCS & gimme_xs){
  //Set Beam particle
  BeamL.SetPxPyPzE(0,0,-EneL,EneL);
  BeamH.SetPxPyPzE(0,0,EneH,TMath::Sqrt(Mp*Mp+EneH*EneH));
  s=(BeamL+BeamH).Mag2();
  ...
  for (int ev=0;ev<nevents;ev++){
    ...
    if (s1_max>s1_min){
      ...
      q2p=rando->Uniform(q2pmin,TMath::Power(TMath::Sqrt(s2)-Mp,2)); //Knowing the s2, q2' and MP, we can derive the momentum of the proton in the cms
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        GenerateEvents(El_beam, p_beam, nevents, ndec, gimme_xs);
