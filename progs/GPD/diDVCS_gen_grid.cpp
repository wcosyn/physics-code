...
void GenerateEvents(double EneL, double EneH, int nevents, int ndec, DiDVCS & gimme_xs, complex<double> ****grid){
  //Set Beam particle
  BeamL.SetPxPyPzE(0,0,-EneL,EneL);
  BeamH.SetPxPyPzE(0,0,EneH,TMath::Sqrt(Mp*Mp+EneH*EneH));
  s=(BeamL+BeamH).Mag2();
    ...
    if (s1_max>s1_min){
      ...
      if(tN>=-0.55){   //limit tNrange...
        for (int dec=0;dec<ndec;dec++){
            ...
            double result_interp = getCross_DiDVCS_interp(2.,trho,tN,q2,q2p,s2,s,y,grid);
            cout << result_interp << " " << results[1] << " " << abs(result_interp - results[1])/results[1] << endl;
