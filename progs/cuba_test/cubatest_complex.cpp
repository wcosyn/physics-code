...
void test_vegas() {
	const int ndim = 4;
	numint::mdfunction<std::complex<double>,ndim> mymdf;
	mymdf.func = unit_sphere_complex<ndim>;
	...
	std::cout << "Vegas: res = " << integral << " error is " << error << " prob is " << prob << " neval " << neval << " fail " << fail << std::endl;
}
