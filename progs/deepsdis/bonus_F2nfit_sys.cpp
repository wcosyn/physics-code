//program to do on-shell extrapolation of F2n using the bonus data, and specifically to estimate the systematic error on the results
...
//values are computed by weighted averaging over all the angles
//we estimate systematic error by raffling the normalization with its error and the data with its stat and sys error.  
//rest is as before, we do N tries and look at average and variance.
...
void get_data(double &data, double& syserror, double& staterror, int beamindex, int Qindex, int Windex, int psindex, int cosindex);
...
int main(int argc, char *argv[])
{
  ...
  generalfile.open(outdir+wf+"."+strucname+".pw"+std::to_string(pw)+
	  ".lc"+std::to_string(lc)+".q2dep"+std::to_string(q2dep)+".off"+std::to_string(offshellset)+".fitsrigsys4.dat");
...
//   int N=1E05;
//   for(int i=0;i<N;i++) values.push_back(gsl_ran_gaussian(r,1.));
//   double avg=0., sigma=0.;
//   for(int i=0;i<N;i++) avg+=values[i];
//   avg/=values.size();
//   for(int i=0;i<N;i++) sigma+=pow(values[i]-avg,2.);
//   sigma=sqrt(sigma/values.size());
//   cout << avg << " " << sigma  << endl;
//   exit(1);
  
  for(int Qindex=1;Qindex<3;Qindex++){
    double Q2 = 0.5*(data::Q2[Qindex]+data::Q2[Qindex+1]);
    for(int Windex=4;Windex>=0;Windex--){
      ...
      vector<double> randomdistr;
      for(int gaan=0;gaan<rand_it;gaan++){
	thetavals.clear();
	thetasigma.clear();
	for(int cosindex=0;cosindex<10;cosindex++){
	  ...
	  data_array.clear();
	  error_array.clear();
	  double sys_bench= gsl_ran_gaussian(r,1.);  //benchmark random value for sys error
	  for(int psindex=0;psindex<4;psindex++){
	    ...
	    double result, staterror, syserror, norm, errornorm;
  ...
  // 	  cout << norm << endl;
	    get_data(result, staterror, syserror, beamindex,Qindex,Windex,psindex,cosindex);
...
// 	      cout << result*ratio/norm << " " << staterror*ratio/norm << " " ;
  // 	    error= error/result+errornorm/norm;
	      norm+=gsl_ran_gaussian(r,errornorm)+gsl_ran_gaussian(r,0.03*errornorm);
	      result+=gsl_ran_gaussian(r,staterror)+syserror*sys_bench;
	      result*=ratio/norm;
	      staterror*=ratio/norm;
// 	      cout << result << " " << staterror << endl;
  // 	    cout << result <<  " " << error << " " << error/result << endl;
	      tarray.push_back(tprime*1.E-06);
	      data_array.push_back(result);
	      error_array.push_back(staterror);
	    }
  // 	  else cout << ratio << endl;
	    sys_bench= gsl_ran_gaussian(r,1.); //different beam different systematics?
	    if(Qindex>0){
	      beamindex=1;
...
// 		cout << result*ratio/norm << " " << staterror*ratio/norm << " " ;
		...
		result*=ratio/norm;
		staterror*=ratio/norm;
// 		cout << result << " " << staterror << endl << endl;
   // 	      cout << result <<  " " << error << " " << error/result << endl;
		tarray.push_back(tprime*1.E-06);
		data_array.push_back(result);
		error_array.push_back(staterror);
...
// 	      << f/(tarray.size()-3) << endl;
	    ...
	    cout << gMinuit->GetParameter(2) << " " << eparab[2] << endl << endl;
	    delete gMinuit;
	  }
	}
...
// 	cout << gaan << " " << Qindex << " " << Windex << " " << avgform << " " << sigmaform << endl;
      ...
      double avg=0.,sigma=0.;
      for(int i=0;i<rand_it;i++) avg+=randomdistr[i];
      avg/=randomdistr.size();
      for(int i=0;i<rand_it;i++) sigma+=pow(randomdistr[i]-avg,2.);
      sigma=sqrt(sigma/randomdistr.size());
//       cout << "BLAAAA " << Qindex << " " << Windex << " " << avg << " " << sigma << endl;
      generalfile << xref << " " << Q2*1.E-06 << " " << Wprime << " " << avg << " " << sigma 
	<< " " << sigma/F2refp << " " << randomdistr.size() << " " << F2refn << " " << F2refp << endl;
//       exit(1);
...
void  get_data(double &data, double& syserror, double& staterror, int beamindex, int Qindex, int Windex, int psindex, int costhetaindex){
  if(beamindex==0){ 
    staterror=data::bonusdata4[Qindex][Windex][psindex][costhetaindex][3];
    syserror=data::bonusdata4[Qindex][Windex][psindex][costhetaindex][2];
    data=data::bonusdata4[Qindex][Windex][psindex][costhetaindex][1];
  }
  else{
    staterror=data::bonusdata5[Qindex-1][Windex][psindex][costhetaindex][3];
    syserror=data::bonusdata5[Qindex-1][Windex][psindex][costhetaindex][2];
