...
//continuum contribution with gaussian shape
//make disinc
// run ../../bin/disinc x Q2[GeV^2] deuteronwf structfunc widths(interval = 2*width)[MeV]
// fi: ../../bin/disinc 0.1 5. paris SLAC 300 500 700
...
int main(int argc, char *argv[])
{
    ...
    for(int proton=0;proton<=1;++proton){
      ...
      double f1,f2;
      Dinc.calc_F2DincFSI_Gauss(f1,f2,Q2,x,centrals,widths);
      fsi1+=f1;
      fsi2+=f2;
      if(!Dinc.getDeutwf()->IsRelativistic()){
	Dinc.setOffshell(3);
	Dinc.calc_F2DincFSI_Gauss_off(f1,f2,Q2,x,centrals,widths);
	fsioffsuppr1+=f1;
	fsioffsuppr2+=f2;
      }
    }
