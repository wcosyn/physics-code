...
struct Ftor {
  ...
  static void exec(const numint::array<double,2> &x, void *param, numint::vector_d &ret) {
    Ftor &p = * (Ftor *) param;
    p.f(ret,x[0],x[1],*p.pObs,*p.pNucleus,*p.lepton,p.E_N,p.costhetaN,p.current,p.cthmax,
  ...
  int maxEvalweakamp;
  string exp;
  void (*f)(numint::vector_d &, double Ein, double costhetamu,
	    WeakQECross& pObs, MeanFieldNucleusThick &pNucleus, TLeptonKinematics &lepton,
	double E_out, double costhetaN, int current, double *cthmax, double max_initial_nucl_mom, double min_final_nucl_mom,
...
void adap_intPm(numint::vector_d &, double E_in, double costhetamu,
		WeakQECross& pObs, MeanFieldNucleusThick &pNucleus, TLeptonKinematics &lepton,
		double E_N, double costhetaN, int current, double *cthmax, double max_initial_nucl_mom, 
...
//   double Q2=atof(argv[1])*1.E06;
  double T_N=atof(argv[1]); // muon momentum along beam [MeV]
  double costhetaN=atof(argv[2]); // muon momentum perp to beam [MeV]
...
//   double omega=Q2/(2.*MASSP*Bjx);
  ...
  double E_N = T_N + MASSn;
  double pN = sqrt(E_N*E_N-MASSn*MASSn);
  ...
  for(int i=1;i<1000;i++){
    double E_in=maxbeam*0.001*i; //possible incoming lepton energies, start from lowest value up to max incoming
