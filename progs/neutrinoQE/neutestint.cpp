...
struct Ftor {
  ...
  static void exec(const numint::array<double,4> &x, void *param, numint::vector_d &ret) {
    Ftor &p = * (Ftor *) param;
    p.f(ret,x[0],x[1],x[2],x[3],*p.pObs,*p.pNucleus,*p.lepton,p.current,p.cthmax);
  }
  WeakQECross *pObs;
...
//   double costhetamu;
  int current;
  double *cthmax;
  void (*f)(numint::vector_d &, double E_out, double costhetamu, double Ein, double costhetacm,
	    WeakQECross& pObs, MeanFieldNucleusThick &pNucleus, TLeptonKinematics &lepton,
	    int current, double *cthmax);
...
void adap_intPm(numint::vector_d &, double E_out, double costhetamu, double E_in, double costhetacm,
		WeakQECross& pObs, MeanFieldNucleusThick &pNucleus, TLeptonKinematics &lepton,
		int current, double *cthmax);
...
//   double Q2=atof(argv[1])*1.E06;
  double T_mu_min=atof(argv[1]);
  double costhetamu_max=atof(argv[2]);
  double costhetamu_min=costhetamu_max-0.1;
...
//   double omega=Q2/(2.*MASSP*Bjx);
  ...
  TLeptonKinematics *lepton = TLeptonKinematics::CreateWithCosScatterAngle(TLeptonKinematics::muon,costhetamu_max);

  double E_out_min=T_mu_min+lepton->GetLeptonMass();
  double E_out_max=E_out_min+100.;
  ...
  //find reasonable integration limits, once for each of the 4 extrema in the final lepton energy and angle bins
  { E_low=E_out_max;
   E_high=E_out_max;
   lepton->SetCosScatterAngle(costhetamu_max);
//     for(int shell=0;shell<Nucleus.getTotalLevels();shell++) {cthmax[shell]=-1.;cthmin[shell]=1.;}
//     max=-1;min=1.;
  for(int i=1;i<1000;i++){
    double E_in=E_out_min+(3.E03-E_out_min)*0.001*i;
    double omega=E_in-E_out_min;
    double Q2=2.*E_in*E_out_min*(1.-sqrt(1.-lepton->GetLeptonMass()*lepton->GetLeptonMass()/(E_out_min*E_out_min))*costhetamu_max)
...
//     cout << E_in << " " << omega << " " << Q2*1.E-06 << " " << x <<  endl;
	...
	double tempmax=-1., tempmin=1.;
      ...
      if(getBound(tempmax,tempmin,Nucleus,*lepton,E_in,E_out_min,costhetamu_max,shell)<300.){ 
	if(!switchlow){
...
// 	cout << shell << " " << kin.GetPklab() <<" " << cthmin[shell] << " " << cthmax[shell] << endl;
  ...
  { lepton->SetCosScatterAngle(costhetamu_max);
  for(int i=1;i<1000;i++){
    double E_in=E_out_max+(3.E03-E_out_max)*0.001*i;
    double omega=E_in-E_out_max;
    double Q2=2.*E_in*E_out_max*(1.-sqrt(1.-lepton->GetLeptonMass()*lepton->GetLeptonMass()/(E_out_max*E_out_max))*costhetamu_max)
	-lepton->GetLeptonMass()*lepton->GetLeptonMass();
    double x=Q2/(2.*MASSP*omega);
//     cout << E_in << " " << omega << " " << Q2*1.E-06 << " " << x <<  endl;
    for(int shell=0;shell<Nucleus.getTotalLevels();shell++) {
	...
	double tempmax=-1., tempmin=1.;
      ...
      //anything above 300 MeV contribution will be negligible
      if(getBound(tempmax,tempmin,Nucleus,*lepton,E_in,E_out_max,costhetamu_max,shell)<300.){ 
	if(!switchlow){
	  cout<< E_in << " " << kin.GetPklab() << " " << shell << endl;
	  switchlow=1;
	  E_low=E_in;
...
// 	cout << shell << " " << kin.GetPklab() <<" " << cthmin[shell] << " " << cthmax[shell] << endl;
  ...
  { lepton->SetCosScatterAngle(costhetamu_min);
  for(int i=1;i<1000;i++){
    double E_in=E_out_min+(3.E03-E_out_min)*0.001*i;
    double omega=E_in-E_out_min;
    double Q2=2.*E_in*E_out_min*(1.-sqrt(1.-lepton->GetLeptonMass()*lepton->GetLeptonMass()/(E_out_min*E_out_min))*costhetamu_min)
	-lepton->GetLeptonMass()*lepton->GetLeptonMass();
    double x=Q2/(2.*MASSP*omega);
//     cout << E_in << " " << omega << " " << Q2*1.E-06 << " " << x <<  endl;
    for(int shell=0;shell<Nucleus.getTotalLevels();shell++) {
	...
	double tempmax=-1., tempmin=1.;
      ...
      //anything above 300 MeV contribution will be negligible
      if(getBound(tempmax,tempmin,Nucleus,*lepton,E_in,E_out_min,costhetamu_min,shell)<300.){ 
	if(!switchlow){
	  cout<< E_in << " " << kin.GetPklab() << " " << shell << endl;
	  switchlow=1;
	  E_low=E_in;
...
// 	cout << shell << " " << kin.GetPklab() <<" " << cthmin[shell] << " " << cthmax[shell] << endl;
  ...
  {lepton->SetCosScatterAngle(costhetamu_min);
   for(int i=1;i<1000;i++){
    double E_in=E_out_max+(3.E03-E_out_max)*0.001*i;
    double omega=E_in-E_out_max;
    double Q2=2.*E_in*E_out_max*(1.-sqrt(1.-lepton->GetLeptonMass()*lepton->GetLeptonMass()/(E_out_max*E_out_max))*costhetamu_min)
	-lepton->GetLeptonMass()*lepton->GetLeptonMass();
    double x=Q2/(2.*MASSP*omega);
//     cout << E_in << " " << omega << " " << Q2*1.E-06 << " " << x <<  endl;
    for(int shell=0;shell<Nucleus.getTotalLevels();shell++) {
	...
	double tempmax=-1., tempmin=1.;
      ...
      //anything above 300 MeV contribution will be negligible
      if(getBound(tempmax,tempmin,Nucleus,*lepton,E_in,E_out_max,costhetamu_min,shell)<300.){ 
	if(!switchlow){
	  cout<< E_in << " " << kin.GetPklab() << " " << shell << endl;
	  switchlow=1;
	  E_low=E_in;
...
// 	cout << shell << " " << kin.GetPklab() <<" " << cthmin[shell] << " " << cthmax[shell] << endl;
  ...
  unsigned neval = 0;
  numint::array<double,4> lower = {{E_out_min,costhetamu_min,E_low,min}};
  numint::array<double,4> upper = {{E_out_max, costhetamu_max,E_high,max}};
...
//integrandum
void adap_intPm(numint::vector_d & results, double E_out, double costhetamu, double E_in, double costhetacm,
...
// 	cout << "bla " << E_in << " " << costhetacm << " " << shell << " " << pm << " " << kin.GetPk() << endl;
	...
	double result=pObs.getDiffWeakQECross(kin,current,1,0,0,1,shell,0.,2E04,WeakQECross::ctheta_cm,1);
